...
Note that the MTU command-line argument (!) for the {\tt router} {\em
  includes} the Ethernet header (see check in {\tt forward\_to})
largely because the code was written to also enable VLANs and thus
different Ethernet header sizes are possible. So to achieve the
traditional Ethernet MTU of 1500 bytes, you would need to specify 1514
on the command-line --- and subtract the size of the Ethernet header
when generating the ICMP message.
